...
class ContainerService(Flask):
    def __init__(self, *args, **kwargs):
        ...
        fh = RotatingFileHandler(
            CONTAINER_CONFIG["logging"]["rot_fh_fname"],
            backupCount=CONTAINER_CONFIG["logging"]["rot_fh_count"],
            maxBytes=CONTAINER_CONFIG["logging"]["rot_fh_max_file_size"])
        ...
        # NOTE: Important for the dumper thread to be started from the
        # container (i.e. Flask subclass) else it won't work
        self._dumper_thread = threading.Thread(
            name="DumperThread",
...
if __name__ == "__main__":
    container_service.run(
        debug=CONTAINER_CONFIG["debug"], use_reloader=False,
