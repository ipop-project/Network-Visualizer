...
var createGraph = function(ipopObj) {
    ...
    var cy = window.cy = cytoscape({
        container: document.getElementById('nodes-content'),
        style: [{
                selector: 'node',
                style: {
                    width: 36.37,
    ...
    $("#refresh-icon").click(function() {
        cy.zoom(1)
        cy.pan({ x: 0, y: 0 })
        cy.center(cy.$("#nodes-content"))
    })

    $("#nodes-content").ready(function() {
        cy.zoom(1)
        cy.pan({ x: 0, y: 0 })
        cy.center(cy.$("#nodes-content"))
        $("#loading").hide();
    ...
    cy.on('click', function(event) {
        ...
        if (event.target !== cy) {
            ...
            if (!ipopObj.getNodeIDList().includes(objID)) {
                ...
                $("#node-name").html('Node: <strong>' + nodeName + '</strong>');
