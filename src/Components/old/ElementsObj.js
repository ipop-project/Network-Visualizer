export default class ElementsObj {
    ...
    constructor(nodes, links) {
        ...
        this.addNodeElement = (id) => {
            ...
            elementObj.push(JSON.parse(`{"group":"nodes","data": {"id": "${nodeDetails.id}","label": "${nodeDetails.name}","state":"","type":"","coordinate":"${this.getCoordinate(id)}"}}`));
        }
        ...
        this.addLinkElement = (src, id) => {
            ...
            elementObj.push(JSON.parse(`{"group":"edges","data": { "id":"${linkDetails.id}" ,"label":"${linkDetails.name}","source": "${linkDetails.source}","target": "${linkDetails.target}","state":"","type":"${linkDetails.type}","color":"${linkColor}"}}`));
        }
